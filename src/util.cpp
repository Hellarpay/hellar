...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            if(ptrCategory->count(string("hellar"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\HellarCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\HellarCore
    // Mac: ~/Library/Application Support/HellarCore
    // Unix: ~/.hellarcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "HellarCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/HellarCore";
#else
    // Unix
    return pathRet / ".hellarcore";
#endif
#endif
}
